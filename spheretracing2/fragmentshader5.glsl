...
float shiftquartic(inout float a, inout float b,inout float c, inout float d, inout float e){
    //return -b/(4*a);
    //return -(4*d)/e;
    double x=0;
    double shift=0;
    ...
    /*e= float((e + shift * (d + shift * (c + shift * (b + shift * a))))/a);
    d= float((d + shift * (2.0 * c + shift * (3.0 * b + shift * 4.0 * a)))/a);
    c= float((c + shift * (3.0 * b + shift * 6.0 * a))/a);
    b= float((b + shift * (4.0 * a))/a);
    a= 1;*/
    return float(shift);
...
vec4 solveQuartic(float a, float b, float c, float d, float e) {
    ...
    //float shift=shiftquartic(a,b,c,d,e);
...
float raymarch(vec3 rayDir, inout vec3 rayOrigin) {
    ...
    float x1=rootpolysquartic();
    float shift=shiftquartic(polys[0][4],polys[0][3],polys[0][2],polys[0][1],polys[0][0]);
    shift=max(shift,0);
    compilepolys(rayOrigin+rayDir*shift,rayDir);
    float x2=rootpolysquartic();
    /*if(isinf(x)){
        raymarch2(rayDir,rayOrigin);
    }*/
    rayOrigin += rayDir *((x2+shift));
    return 0;
}
float raymarch2(vec3 rayDir, inout vec3 rayOrigin) {
    compilepolys(rayOrigin,rayDir);
