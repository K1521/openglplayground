...
def stringifynodespy(endpoint):
    ...
    fundec=f"""
def calcpoly(x):
    return poly[0]+poly[1]*x+poly[2]*x**2/2+poly[3]*x**3/6+poly[4]*x**3/24
...
for i in range(4):
    ...
    derivatives.append([p.backpropergation([a])[0] for p in derivatives[-1]])
...
with open("./spheretracing2/scene.glsl","w")as f:
    f.write(fundec)
#print(fundec)
