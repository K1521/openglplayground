...
void main() {
    ...
    int i=marchingcubestable[tableindex*13];
    triangles[idot(pos0, delta)*12]=vec3(tableindex,0,0);
 

}
""".replace("{marchingcubestable}",tableconvert.tablestringLinear).replace("{threadnum1}",str(threadnum[0])).replace("{threadnum2}",str(threadnum[1])).replace("{threadnum3}",str(threadnum[2]))
