...
tablepadded=[row+[-1]*(max_row_length-len(row)) for row in table]
# Build the GLSL code for the TriangleTable

tablestringarray="{\n"+",\n".join(
    "{" + ",".join(map(str, row)) + "}" 
    for row in tablepadded
)+"\n};"
tablestring = f"uniform int marchingcubestable[256][{max_row_length}] = "+tablestringarray
tablestringLinear = f"uniform int marchingcubestable[{256*max_row_length}] = {{" + ",".join(map(str, [v for row in tablepadded for v in row])) + "};" 
